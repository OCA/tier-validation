...
class TierReview(models.Model):
    ...
    def action_bulk_validate(self):
        ...
        for (model, res_id), reviews in by_key.items():
            ...
            validated[model] = validated.get(model, record.browse()) | record
        # Update the counter once for the whole batch, not once per record.
        # ``_update_counter`` re-promotes the remaining reviews and pushes a
        # bus notification, and the systray answers every notification with a
        # recount of its badge. Per record, approving a selection of N
        # documents fans out to N notifications per reviewer -- multiplied by
        # each reviewer's open tabs -- which is exactly the pattern bulk
        # approve is meant to make routine. Batched, the selection costs one
        # notification per reviewer however large it is.
        for records in validated.values():
            records._update_counter({"review_deleted": True})
        return True
